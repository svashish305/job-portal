...
function getById(req, res, next) {
    jobService.getById(req.params.id)
        .then(job => (job && (job.userId === req.user.id || req.user.role === Role.Admin)) ? res.json(job) : res.sendStatus(404))
        .catch(next);
}

function create(req, res, next) {
    jobService.create({...req.body, userId: req.user.id, updated: Date.now()})
